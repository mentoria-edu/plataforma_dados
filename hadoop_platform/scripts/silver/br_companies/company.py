...
HUDI_CONFIGS = {
    ...
    "hoodie.datasource.write.recordkey.field": "cnpj,_hash",
...
def clean_bronze_data(spark: SparkSession, table: str) -> DataFrame:
    ...
    df = df.select(
        trim(col("cnpj")).alias("cnpj"),
        trim(col("company_name")).alias("company_name"),
        trim(col("legal_nature")).alias("legal_nature"),
        trim(col("responsible_qualification")).alias("responsible_qualification"),
        col("share_capital").cast(DoubleType()).alias("share_capital"),
        coalesce(trim(col("company_size")), lit("00")).alias("company_size"),
        trim(col("federative_entity")).alias("federative_entity"),
        col("_batch_timestamp").cast("timestamp"),
...
def get_data_to_update(
    ...
    join_cond = [
        target_df._hash != source_df._hash,
...
def main() -> None:
    ...
    df_source_cleaned = clean_bronze_data(spark, BATCH_TABLE)
    ...
    df_source_cleaned.write.format("hudi").mode("overwrite").options(**HUDI_CONFIGS).saveAsTable(TARGET_TABLE)
